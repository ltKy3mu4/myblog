...
create index if not exists name_idx on tags (name);
...
create index if not exists post_id_idx on posts_tags (post_id);

create index if not exists tag_id_idx on posts_tags (tag_id);
